...
def build_prompt(emotion: str) -> str:
    bucket = config.GOEMOTIONS_TO_BUCKET.get(emotion.lower(), "neutral")
    direction = COMFORT_STYLES.get(bucket, COMFORT_STYLES["neutral"])
    return f"{direction}, {STYLE_SUFFIX}"


def build_prompt_epd(emotion: str) -> str:
    bucket = config.GOEMOTIONS_TO_BUCKET.get(emotion.lower(), "neutral")
    direction = EPD_COMFORT_STYLES.get(bucket, EPD_COMFORT_STYLES["neutral"])
...
def generate_image_local(prompt: str, emotion: str = "neutral") -> str:
    ...
    if _pipe is None:
        ...
        # Swapping in DPMSolverMultistepScheduler here throws an IndexError on the
        # final step with this diffusers version (0.36) -- sigmas array is one short
        # of what multistep_dpm_solver_second_order_update looks up at step 35/35.
        # The pipeline's default scheduler doesn't hit this, so keep it.
