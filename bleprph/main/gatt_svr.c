                ...
                struct ble_gatt_access_ctxt *ctxt, void *arg)
{
    ...
    case BLE_GATT_ACCESS_OP_WRITE_CHR: //BLE WRITE
        if (conn_handle != BLE_HS_CONN_HANDLE_NONE) {
        ...
        uuid = ctxt->chr->uuid;
        if (attr_handle == gatt_svr_chr_val_handle) {
            rc = gatt_svr_write(ctxt->om, //스마트폰에서 전송한 데이터 저장
                                sizeof(gatt_svr_chr_val),
                                sizeof(gatt_svr_chr_val),
            ...
            {
                MODLOG_DFLT(ERROR,
                            "Failed to write characteristic; rc=%d\n",
                            rc);
                return rc;
            }

            MODLOG_DFLT(INFO, "BLE RX value=0x%02X\n", (unsigned int)gatt_svr_chr_val);
            ...
            if (gatt_svr_chr_val == 0x01)
            {
                ...
                if (epd_ret != ESP_OK) {
                    ESP_LOGE(
                        "BLE",
                        "Failed to request EPD update: %s",
                        esp_err_to_name(epd_ret)
                    );
                } else {
                    ESP_LOGI(
                        "BLE",
                        "EPD default image request queued"
                    );
                }
            }
            else
